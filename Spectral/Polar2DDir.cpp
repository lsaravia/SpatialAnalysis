...
void SpectralAnalysis::Polar2DDir(
					...
					simplmat<double>& pSp,		// Polar spectra in Bins
					simplmat<double>& cpSp		// Counts of Polar spectra in Bins
					)
{
	if( n != f.getCols() )
	{
	...
	simplmat <double> zp(outt,20);

	pSp.resize(outt,20,0.0);		// Polar spectra by bins
	cpSp.resize(outt,20,0.0);		// Counts
...
//
	...
	for(l=0;l<outt;l++)
	{
		pSp(l,0)  += pSp(l,18)+pSp(l,19);
		cpSp(l,0) += cpSp(l,18)+cpSp(l,19);
		zp(l,0) += zp(l,18)+zp(l,19);
	}
	...
	for(l=0;l<outt;l++)
	{
		...
		for(j=0;j<18;j++)
			pSp(l,j) = pSp(l,j)/(zp(l,j) + 1.0e-30);
	}
//
